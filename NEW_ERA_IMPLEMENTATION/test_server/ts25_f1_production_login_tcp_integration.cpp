...
int main(){
 PacketCryptoSM sm; std::string err; const std::string keyfile="/tmp/ts25_enc1.dat"; {std::ofstream f(keyfile,std::ios::binary); assert(f); f.write(reinterpret_cast<const char*>(enc1),sizeof(enc1));} assert(sm.LoadKeysFromFile(keyfile,&err,0)); crypto::PacketCryptoSM dec; crypto::PacketCryptoSM::Keys dk; assert(DeriveDecryptKeysFromEncryptKeys(sm.DebugKeys(),dk,err)); dec.SetKeys(dk);
 ...
 std::vector<uint8_t> response(5); bool server_ok=false; std::thread srv([&]{int cs=accept(ls,nullptr,nullptr);assert(cs>=0);std::vector<uint8_t> rx(79);assert(recvn(cs,rx));std::string de;DecodedLoginRequest d;assert(DecodeF101(rx,dec,d,de));ServerLoginPolicy p;p.expected_version="12345";p.expected_protocol_serial=std::string("ABCDEFGHIJKLMNOP",16);assert(ValidateServerLoginRequest(d,p,de));uint8_t out[5];assert(BuildLoginResultC1(JoinServerResult::Success,out));response.assign(out,out+5);server_ok=send(cs,(const char*)out,5,0)==5;closefd(cs);});
