...
inline bool UndoLoginStreamXor(uint8_t* logical,size_t size,std::string& error){if(!logical||size!=50){error="logical F1:01 size must be 50";return false;}uint8_t previous=logical[2];for(size_t i=3;i<49;++i){uint8_t wire=logical[i];logical[i]=static_cast<uint8_t>(wire^previous^crypto::kXorFilter32[i%32]);previous=wire;}return true;}
