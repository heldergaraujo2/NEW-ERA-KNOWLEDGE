...
inline bool BuildC3(const LoginRequestInput& in,crypto::PacketCryptoSM& sm,std::vector<uint8_t>& out,std::string& error){
 if(!sm.KeysLoaded()){error="SimpleModulus Enc1 keys not loaded";return false;}
 std::vector<uint8_t> logical;
 if(!BuildLogicalC1(in,logical,error))return false;
 // SendPacket inserts packet serial at C1 byte [1] before encryption. C1 byte [0] is not encrypted.
 const int encrypted_length=int(logical.size())-1;
 const int blocks=(encrypted_length+7)/8;
 std::vector<uint8_t> cipher(size_t(blocks)*11);
 const int n=sm.Encrypt(cipher.data(),logical.data()+1,encrypted_length);
 if(n<0){error="SimpleModulus encryption failed";return false;}
 if(n>=254){error="ciphertext requires C4 framing";return false;}
 out.clear();out.reserve(size_t(n)+2);
 out.push_back(0xC3);out.push_back(uint8_t(n+2));
 out.insert(out.end(),cipher.begin(),cipher.begin()+n);
 return true;
}
