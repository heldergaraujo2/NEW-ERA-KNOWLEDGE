// NEW-ERA — loopback_harness.cpp (1.3-B) — F3:0x01 Create Character — WIRE-REAL C1
// Prova end-to-end LOCAL com socket REAL — SOMENTE 127.0.0.1 (hardcode; sem args).
//   SERVER stub: envia kRespServer (golden C1 19 B) -> recebe request C1 PLAIN
//                (wire real 15 B) -> memcmp vs kReqExpected -> close.
//   CLIENT MVP : decodifica response C1 (sem decrypt) -> ParseC1_F3_01 com
//                asserts (result/id/index/level/class) -> envia request do
//                builder WIRE REAL (BuildC1_F3_01_CreateCharRequestWire).
...
static void ServerThread(std::promise<int> portPromise) {
    ...
    printf("[server] conectado; enviando RESP golden C1 (%zu B)\n", loopback_f3c::kRespSize);
    if (!SendAll(c, loopback_f3c::kRespServer.data(), loopback_f3c::kRespSize)) { ::close(c); return; }
    ...
    if (pkt.size() != loopback_f3c::kReqSize ||
        std::memcmp(pkt.data(), loopback_f3c::kReqExpected.data(), loopback_f3c::kReqSize) != 0) {
        size_t n = pkt.size() < loopback_f3c::kReqSize ? pkt.size() : loopback_f3c::kReqSize;
        size_t diff = n;
        for (size_t i = 0; i < n; ++i) if (pkt[i] != loopback_f3c::kReqExpected[i]) { diff = i; break; }
        printf("[server] REQUEST DIVERGENTE do golden (tam %zu vs %zu; 1º offset: %zu)\n",
               pkt.size(), loopback_f3c::kReqSize, diff);
        ::close(c);
        return;
    }
...
static bool ClientThread(int port) {
    ...
    // 1) response golden C1 -> decode (sem decrypt) + parse create char
    std::vector<uint8_t> resp(loopback_f3c::kRespSize);
    ...
    if (!mvp::DecodeAndParseMvpPacket(resp, smAny, ver, out, err, /*streamXored=*/false, &plainC1) ||
        out.head != 0xF3 || out.sub != 0x01) {
        printf("[client] decode RESP falhou\n"); ::close(fd); return false;
    }
    mvp::ParsedCreateChar cc; std::string perr;
    if (!mvp::ParseC1_F3_01_CreateCharResponsePlain(plainC1, cc, perr) ||
        (int)cc.result != loopback_f3c::kExpectedResult ||
        (int)cc.index != loopback_f3c::kExpectedIndex ||
        cc.level != loopback_f3c::kExpectedLevel ||
        cc.serverClass != loopback_f3c::kExpectedClass ||
        std::string(cc.id) != loopback_f3c::kExpectedId) {
        printf("[client] parse divergente do expected_parse\n"); ::close(fd); return false;
    }
    printf("[client] decoded RESP C1: result=%u id=%s index=%u level=%u class=%u\n",
           cc.result, cc.id, cc.index, cc.level, cc.serverClass);

    // 2) request C->S com o builder WIRE REAL (id/class/skin do golden)
    auto req = mvp::BuildC1_F3_01_CreateCharRequestWire(loopback_f3c::kExpectedId,
                                                        /*cls=*/3, /*skin=*/2);
...
int main() {
    ...
    printf("LOOPBACK F3:01 WIRE OK: RESP-C1(result/id/index/level)->REQ-C1-wire(match)\n");
    return 0;
}
