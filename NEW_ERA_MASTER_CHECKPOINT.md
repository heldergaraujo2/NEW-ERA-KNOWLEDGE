...
| **1.3-A** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2, C1 nativo)** | Ledger §65 + [FILE] `NEW_ERA_PROTOCOL_MVP_F3_06_ADDPOINT_SPEC.md` + [DIR] `test_vectors_f3_06/`+`loopback_f3_06/` | F3:06: REQ C1 5 B pointType (:1189-:1195) · RESP 11 B **Result nibble-packado** (:6203-:6227; Max dual-use); call-site [NOT RECOVERED]; sem invenção de campos |
| **1.3-B** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2, C1 nativo)** | Ledger §66 + [FILE] `NEW_ERA_PROTOCOL_MVP_F3_01_CREATECHAR_SPEC.md` + [DIR] `test_vectors_f3_01/`+`loopback_f3_01/` | F3:01: REQ C1 15 B ID10+**classSkin nibble** (:298-:308) · RESP 19 B (:376-:386; Result 1/0/2 :625/:664/:666); builder c/ bounds; golden 0x32 conferido pós-decode |
| **1.3-C** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; 1º pacote DUPLO-SISTEMA)** | Ledger §67 + [FILE] `NEW_ERA_PROTOCOL_MVP_JOIN_GAME_SPEC.md` + [DIR] `test_vectors_join_game/`+`loopback_join_game/` | JoinGame: TX **olc::net 16 B** [id=0x000B:u16 LE][size=10:u32 LE][ID10] (:322-:330; ProtocolAsio.h :33-:37 fetch@580472e) SEM C1/XOR · ponte GS :122-:131 → F3:03 · RX **C1 66 B (0x42)** PRECEIVE_JOIN_MAP_SERVER (:405-:452; exp 8×B BE; 12 stats W LE; Gold DW; ângulo (a-1)×45 :980; sem result byte); [NOT RECOVERED] sender GS response + macro spe clássico; spec classe (2) |
| **1.3-D** (FASE 1) | **RESEARCH/COMPAT — SEM MVP (F3:E0 extra não consumido)** | Ledger §68 + [FILE] `NEW_ERA_PROTOCOL_MVP_POSTJOIN_SELFINFO_SPEC.md` (classe (2)) | F3:E0: sender GS provado (:3417/:3458; GAMESERVER_EXTRA==1; 36 campos :3419-:3456; struct [NOT RECOVERED]; irmão F3:E1) · client upstream: 24 subs F3 SEM 0xE0/default ⇒ consumidor = client EXTRA · core: validador header-only payload-opaco |
| **1.3-E** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; C2; 1ª entidade visível)** | Ledger §69 + [FILE] `NEW_ERA_PROTOCOL_MVP_VIEWPORT_SPAWN_SPEC.md` + [DIR] `test_vectors_viewport_13/`+`loopback_viewport_13/` | 0x13 S→C **C2 provado** (cast :2587; PWMSG_HEADER :83-:89): [C2][szH][szL][0x13][count BYTE :202] + PCREATE_MONSTER :588-:600 c/ **stride 10+s_BuffCount** (:2710; MAX_BUFF=16); Key BE &0x7FFF+flags; Type 10-bit; dir=Path>>4 (:2637); "ViewPortNumber" do exemplo NÃO inventado; classe (1) |
| **1.3-F** (FASE 1) | **EXECUTADO — SPEC§BUFFS + HARDEN + GOLDEN + LOOPBACK OK (exit 0×2; stride variável)** | Ledger §70 + [FILE] spec §"BUFFS E STRIDE VARIÁVEL" + [DIR] `test_vectors_viewport_13_buffs/`+`loopback_viewport_13_buffs/` | C2 29 B, 2 entidades strides **11/13** (:2710) c/ buffs **[AA]** e **[01 02 03]** validados byte-a-byte em `SpawnEntity.buffs`; harden **s_BuffCount>16 rejeitado** (:599/:613; `kViewportMaxBuffSlotIndex`); RegisterBuff real (eBuffState, OBJECT*) documentado |
| **1.3-G** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; C2; players/NPCs visíveis)** | Ledger §71 + [FILE] `NEW_ERA_PROTOCOL_MVP_VIEWPORT_CHARACTER_SPEC.md` + [DIR] `test_vectors_viewport_12/`+`loopback_viewport_12/` | 0x12 S→C **C2** (:2169): [C2][sz][0x12][count] + PCREATE_CHARACTER :537-:550 (36 B + n buffs, stride **36+n** :2376) — **Path dual dir/PK** (:2271/:2237), Class&0x07=pose, Equipment[17] opaco (:71); golden **79 B** (cmd dizia 115 — divergência aritmética documentada, evidência prevaleceu); strides 36/38 provados |
| **1.3-H** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; world tick agregado)** | Ledger §72 + [FILE] `NEW_ERA_PROTOCOL_MVP_WORLDSTATE_LOOP_SPEC.md` + [DIR] `test_vectors_worldtick/`+`loopback_worldtick/` | WorldState spawn-only: `entities` key→EntityRecord (kind char/monster) + Apply* 0x12/0x13 **LWW**; frame inválido NÃO altera estado (trunc provado c/ snapshot); key==0 permitida (0..0x7FFF); loopback 3 frames `[len16LE][C2]` (framing = artefato do stub); gerador/harness TWO-TU c/ espelho ODR 1.3-H; frames dos jsons versionados |
| **1.3-I** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; ciclo spawn→delete fechado)** | Ledger §73 + [FILE] `NEW_ERA_PROTOCOL_MVP_VIEWPORT_DELETE_SPEC.md` + [DIR] `test_vectors_viewport_14_delete/`+`loopback_viewport_14_delete/` | 0x14 S→C **C1 (único do ciclo)**: [C1][sz][0x14][count] + PDELETE_CHARACTER 2 B **BE &0x7FFF** (stride FIXO 2 :2846; DeleteFlag b15 dead read; DeleteCharacter tolerante :2839); ciclo 4→2 provado c/ key inexistente ignorada (0x9999→0x1999) e trunc sem efeito |
| **1.3-J** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; ciclo spawn→move→delete fechado)** | Ledger §74 + [FILE] `NEW_ERA_PROTOCOL_MVP_POSITION_UPDATE_SPEC.md` + [DIR] `test_vectors_move_15/`+`loopback_move_15/` | 0x15 S→C **C1 7 B fixo** (PRECEIVE_MOVE_POSITION :892-:898): [C1][07][15][Key BE **sem máscara** :1749][X][Y]; target=posição :1764-:1765; sem dir/angle; move 0x0100→(77,88) provado c/ missed=0/1 (key inexistente) e trunc sem efeito; 0xD4 PACKET_MOVE pendente (header condicional/Path) |
| **1.3-K** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; move com path/angle)** | Ledger §75 + [FILE] `NEW_ERA_PROTOCOL_MVP_PACKET_MOVE_D4_SPEC.md` + [DIR] `test_vectors_move_d4_asio/`+`loopback_move_d4_asio/` | 0xD4 **NO WIRE OLc** (prova tripla :95-:96/:6/:611-:620): [id=0x0007:u16][size:u32][body headerless Key BE sem máscara/X/Y/Path0], mín. 11 B; dir=Path0>>4 :1699; apply não-Hero target=Data :1710-:1711 c/ **x/y inalterados**; legado C1 :13094 = misparse (não implementado); POSITION não migrado (1.3-J válida); move_ok provado (target 77/88, dir 3→90°, missed 0/1, trunc sem efeito) |
| **1.3-L** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; equiv 0x15 provada campo-a-campo)** | Ledger §76 + [FILE] `NEW_ERA_PROTOCOL_MVP_BOTH_POSITION_SPEC.md` + [DIR] `test_vectors_both_position_0006/`+`loopback_both_position_0006/` | BOTH_POSITION=**0x0006** (ordinal 6 enum :7-:26); ponte :92-:94 → ReceiveMovePosition(body.data()) = mesmo handler do 0x15; **divergência §75 RESOLVIDA**: header INCONDICIONAL ⇒ body 7 B contém **hdr3 embutido OPACO** ([NOT RECOVERED]; não lido/não validado) + [Key BE sem máscara][X][Y]; frame [id:u16][size:u32=7][body] = 13 B; x/y/target=Data, dir intocado; both_ok ≡ pkt15_ok provado (SameWorld); miss missed=1; trunc 12 B sem efeito |
| **1.3-M** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; 1º combat signal + túnel)** | Ledger §77 + [FILE] `NEW_ERA_PROTOCOL_MVP_DAMAGE_RX_SPEC.md` + [DIR] `test_vectors_damage_11/`+`loopback_damage_11/` | **PACKET_ATTACK=0x11** (:26; dispatch :13143-:13144 → ReceiveAttackDamage :2984-:3191): **C1 10 B fixo** (PRECEIVE_ATTACK :674-:685) [Key BE **c/ Success=b15 + &0x7FFF** :2995-:2997][Damage BE][DT type&0x3F/b6/b7][Shield BE]; apply lastDamage (c->Hit :3188) + clamp-0 opcionais; **⭐BOTH_MESSAGE olc 0x000C = TÚNEL** (:99-:137 → TranslateProtocol :135, autodetect C1/C2/C3/C4) — inner clássico cru no body; ciclo provado c/ decode completo (raw 0x8100, DT 0xC5) + probe do túnel + 2 trunc sem efeito; irmão self-dano F3:07 c/ bug upstream Shield documentado |
| **1.3-N** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; 1º TX de combate + ciclo REQ→RESP)** | Ledger §78 + [FILE] `NEW_ERA_PROTOCOL_MVP_ATTACK_TX_SPEC.md` + [DIR] `test_vectors_attack_tx_11/`+`loopback_attack_tx_11/` | TX = macro **SendRequestAttack** :518-:527: **C1 7 B fixo** [C1][07][11][KeyH][KeyL][**0x78 AT_ATTACK1**][Dir]; Key BE **sem máscara** :524; C1 plain (Send defaults — não C3); head 0x11 bidirecional (TX 4 B ≠ RX 7 B payload); GS `case PROTOCOL_CODE2` :112-:113 → CGAttackRecv (struct [NOT RECOVERED]; correlação 0x11); transporte C1 direto (SendPacketClassic :146; túnel C→S não existe); loopback RAW 7 B→10 B c/ **memcmp golden no server** + RESP damage ⇒ lastDamage=0x0123 |
| **1.3-O** (FASE 1) | **EXECUTADO — SPEC+IMPL+GOLDEN+LOOPBACK OK (exit 0×2; TX de skill multi-alvo + ciclo REQ→RESP)** | Ledger §79 + [FILE] `NEW_ERA_PROTOCOL_MVP_SKILL_TX_SPEC.md` + [DIR] `test_vectors_skill_db/`+`loopback_skill_db/` | TX = macro **SendRequestMagicAttack** :600-:616 (== inline :618-:633): **C1 0xDB variável** [C1][size=9+3·Count][DB][**Type BE**][x][y][Serial][Count] + por alvo [**Key BE sem máscara**][**SkillSerial**] (12 B @Count=1; ≤82 alvos); **spe.Send(TRUE)** pede encrypt (wrap C3 [NOT RECOVERED]; builder = plain pré-encrypt); Serial def [NOT RECOVERED] (teste documentado); GS **case 0x19** :121-:122 → CGSkillAttackRecv (0xDB ausente no GS — tradução [NOT RECOVERED], análoga join 1.3-C); loopback RAW 12 B→10 B c/ memcmp golden + Count=2 ⇒ 15 B + guard count=0 ⇒ RESP damage lastDamage=0x0123 |
...
- Update: 1.3-V CONFIRMADO (inventário F3:10 C2 variável, item 12B, emissor GS identificado); próximo: 1.3-V P2 (spec+vectors+parser+loopback).
