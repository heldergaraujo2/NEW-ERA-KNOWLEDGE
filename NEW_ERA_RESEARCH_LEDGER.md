...
## 73. FASE 1 — 1.3-I: Viewport delete 0x14 (C1) OK (remove keys; inexistente ignora; erro não altera estado) golden+loopback; binários removidos
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (CICLO SPAWN→DELETE)**: golden aplicou frame12 (C2 79 B) + frame13 (C2 29 B) → **ws=4** → **delete14 (C1 10 B `C10A1403 0100 0101 9999`)** removeu **0x0100 (char) e 0x0101 (monster)** → **ws=2 (ficam 0x0200/0x0202; counts 1 char/1 monster)**; **raw 0x9999 (b15=1) → máscara &0x7FFF :2817 ⇒ key 0x1999 — INEXISTENTE, ignorada sem falhar** (comportamento documentado no golden); **delete14_trunc (9 B, header diz 0x0A) rejeitado** ("0x14: tamanho C1 inconsistente") com estado intocado (snapshot SameState no gerador; size+keys no harness). Loopback: server enviou **4 frames [len:u16 LE][frame]** por socket real (framing = artefato do stub) → client validou o ciclo completo 4→2→intocado.
- **P1/2 (sem rede; classe (1) layout provado)**: pacote = **HeadCode 0x14 S→C, framing C1 — O ÚNICO do ciclo em C1** (cast LPPHEADER_DEFAULT :2795 sobre PHEADER_DEFAULT{PBMSG_HEADER; BYTE Value} H :113-:118; 0x12/0x13 são C2/PWMSG); dispatch :13116-:13119 ("delete characters & monsters" — chars E monsters); entidade = **PDELETE_CHARACTER 2 B** (H :622-:626) c/ **stride FIXO 2** (:2846); Key **BE** :2814 + **&0x7FFF** :2817 → DeleteCharacter :2839 (tolerante a key ausente); **DeleteFlag=b15 = DEAD READ** (:2815 lê, nunca usa — documentado, descartado no parser); UnRegisterBuff antes :2832-:2837; count limite derivado (4+2*count==size). Spec `NEW_ERA_PROTOCOL_MVP_VIEWPORT_DELETE_SPEC.md` 2.820 B sha 3e30a704856eb48a543f2fc85c6bf8489678842690a6e5094d3ccce3c1cc03eb.
- **Impl** (`mvp_login_client.cpp` 1.240 L/62.723 B sha 57c63fe5b669b29ce538096c6a40cf108673cd648e60d41ef8e568ec0bcf73fc, bloco 1.3-I): `ParseViewportDeletePlain_C1` (C1/head 0x14/size/4+2*count; keys BE &0x7FFF; erro claro "0x14:") + `ApplyFrame_DeleteEntities_C1` (parse local; erase tolerante). SYNTAX OK. Ciclo spawn/delete FECHADO no WorldState.
- **Golden** (`test_vectors_viewport_14_delete/`): generate 6.947 B (single-TU; frames 0x12/0x13 lidos dos jsons versionados) · viewport_14_vectors.json **728 B sha da713fe12e4c97d230656679abc40e5a39c4fb2005e12ce585784e0c61b89d73** · viewport_14_vectors.md **496 B sha 0b3fa834583089044a991d309e66ca4518553ab508bafb0006c0048bdc3f1cd0** · compile_and_run.sh 572 B. **Loopback** (`loopback_viewport_14_delete/`): embedded 1.936 B 7e08483d… (sha do json no topo; 4 frames + expected) · loopback_harness.cpp 6.559 B 66aa171c… (single-TU) · compile_and_run.sh 611 B.
- **Binários REMOVIDOS**: genvec_viewport_14 675.128 B sha 33faa18e… · loopback_harness 918.480 B sha 2a0e3350… (liberados 1.593.608 B; workspace 10.781.454 B c/ .git).
- Nota: localhost only; EVIDENCE intacta; nenhum .dat/binário versionado.

## 74. FASE 1 — 1.3-J: Position update 0x15 OK (spawn→move→delete; miss contabilizado; erro não altera estado) golden+loopback; binários removidos
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (CICLO SPAWN→MOVE→DELETE)**: golden aplicou frame12 (C2 79 B) + frame13 (C2 29 B) → **ws=4** → **move_ok (C1 7 B `C10715 0100 4D 58`)** atualizou **0x0100 → (77,88) com targetX/Y=idem (:1762-:1765), missed=0** → **move_miss (`C10715 3333 0909`, key 0x3333 inexistente) → ignored + missed=1, ws segue 4** → **move_trunc (6 B, sem Y) rejeitado** ("0x15: tamanho invalido…") com estado intocado (snapshot) e missed inalterado → **delete14 (C1 10 B, golden 1.3-I) → ws=2**. Loopback: server enviou **6 frames [len:u16 LE][frame]** por socket real → client validou o ciclo completo (moved pos 77/88 + target, missed==1, final 2).
- **P1/2 (sem rede; classe (1) layout provado)**: pacote = **HeadCode 0x15 PACKET_POSITION S→C** (define wsclientinline **:24**; PACKET_MOVE=0xD4 :23); handler **ReceiveMovePosition :1746-:1767** (o + simples: 1 entidade, sem lista/path); framing **C1** (cast :1748) sobre **PRECEIVE_MOVE_POSITION** (WSclient.h :892-:898) = frame FIXO **7 B** [C1][07][15][KeyH][KeyL][X][Y]; **Key BE SEM máscara 0x7FFF** :1749 (nuance vs 0x12/0x13/0x14 — FindCharacterIndex usa valor cru); PositionX/Y :1762-:1763 E TargetX/Y=Data :1764-:1765; **sem dir/angle no pacote** (permanece o do spawn); JumpTime=1 :1766. **Irmão 0xD4 PACKET_MOVE documentado pendente** (ReceiveMoveCharacter :1688-:1745; PMOVE_CHARACTER :611-:620 c/ header CONDICIONAL `#ifndef NEW_PROTOCOL_SYSTEM` e Path[1] variável; TargetAngle=Path[0]>>4 :1699). Spec `NEW_ERA_PROTOCOL_MVP_POSITION_UPDATE_SPEC.md` 2.954 B sha 50f502d0173bf8057f81f671e1c84ee5a83c484015642db387156cb2db22602b.
- **Impl** (`mvp_login_client.cpp` 1.305 L/65.831 B sha d22b0ee4ef7864f2602d8b61bce7da7991bec085217ac8f37842f609fa983e41, bloco 1.3-J): `MoveUpdate{key,x,y}` + `ParsePositionUpdatePlain_C1` (exige C1 7 B exato/head 0x15) + `ApplyFrame_PositionUpdate_C1(frame, ws, err, size_t* missed=nullptr)` (parse local; key existente ⇒ x/y/target=Data c/ dir/angle inalterados; inexistente ⇒ ignora e contabiliza; erro ⇒ estado intocado). SYNTAX OK. **Ciclo spawn/delete/move FECHADO no WorldState.**
- **Golden** (`test_vectors_move_15/`): generate 6.524 B (single-TU; frames 0x12/0x13/delete14 lidos dos jsons versionados) · move_15_vectors.json **697 B sha 4ca08a414dcb852d0411aa4e486b608180e711f7b4e1e5ba1e676f683d44ee25** · move_15_vectors.md **465 B sha 1fbafc86ff5f46099f730df25c6ddd937e2e979a713da17b49ee1c033f5a7fe3** · compile_and_run.sh 522 B. **Loopback** (`loopback_move_15/`): embedded 2.231 B 5de188e9… (sha do json no topo; 6 frames + expected) · loopback_harness.cpp 7.162 B c3daa149… (single-TU) · compile_and_run.sh 576 B.
- **Binários REMOVIDOS**: genvec_move_15 712.624 B sha c8cdccaa… · loopback_harness 963.776 B sha eb326d4a… (liberados 1.676.400 B; workspace 10.955.838 B c/ .git).
- Nota: localhost only; EVIDENCE intacta; nenhum .dat/binário versionado.

## 75. FASE 1 — 1.3-K: BOTH_MOVE (ASIO id=0x0007) OK (apply dir/angle/target; miss contabilizado; trunc não altera estado) golden+loopback; binários removidos
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (BOTH_MOVE no wire real do build)**: golden aplicou frame12 (C2 79 B) + frame13 (C2 29 B) → **ws=4** → **move_ok (olc 11 B `0700 05000000 0100 4D 58 30`)** atualizou **0x0100: targetX/Y=(77,88) :1710-:1711, dir=3 (Path0=0x30>>4 :1699), angleDeg=90 (correlação *45 :2271/:2637), x/y=(10,20) INTOCADOS** (PathFinding2 :1730 interpola no render), missed=0 → **move_miss (`…3333 0909 10`, key 0x3333 inexistente) → ignored + missed=1, ws segue 4** → **move_trunc (10 B: header diz size=5, body 4 B) rejeitado** ("0xD4: frame olc truncado…") com estado intocado (snapshot) e missed inalterado. Loopback: server enviou **5 frames [len:u16 LE][frame]** por socket real → client validou tudo (target/dir/angle/xy-unchanged/missed==1/ws==4).
- **P1/2 (sem rede; classe (1) layout provado p/ o wire ATIVO)**: **DESCOBERTA CENTRAL — o 0xD4 NÃO anda em C1/C2 neste build**: (a) ponte olc **ProtocolSend.cpp :95-:96** `case ProtocolHead::BOTH_MOVE: ReceiveMoveCharacter((BYTE*)msg.body.data())` (sha 609e7e935fc88a02…); (b) **Defined_Global.h :6 define NEW_PROTOCOL_SYSTEM no CLIENTE** (sha 87b5278655bb53f9…); (c) PMOVE_CHARACTER (WSclient.h :611-:620) tem **header CONDICIONAL `#ifndef NEW_PROTOCOL_SYSTEM`** ⇒ body headerless. ⇒ **frame RX = [id:u16=0x0007 BOTH_MOVE LE][size:u32 LE][body ≥5 [KeyH][KeyL][X][Y][Path0]]**, mín. 11 B, 1 entidade, path tail variável (steps além de Path[0] NÃO lidos pelo handler — preservados opacos). Dispatch clássico :13094-:13095 (buffer C1 c/ header) seria **misparse** c/ struct headerless ⇒ legado incoerente, NÃO implementado (anti-invenção). Nuance lateral: BOTH_POSITION :93 alimenta ReceiveMovePosition(body) mas PRECEIVE_MOVE_POSITION (:892-:898) tem header INCONDICIONAL — upstream migrou MOVE e NÃO POSITION (parser 0x15 clássico da 1.3-J permanece). **Key BE SEM máscara** :1691 (grep :1688-:1745 vazio). Spec `NEW_ERA_PROTOCOL_MVP_PACKET_MOVE_D4_SPEC.md` 4.099 B sha b32e7eb009a86b5e700a6d822aa2ef0dee156c55111f2289d01e5d720818a09d.
- **Impl** (`mvp_login_client.cpp` 1.391 L/70.119 B sha c6246993890569f302ba1ff5db57505a0361c853a031c8a6f859175040529f5a, bloco 1.3-K): `kProto_BOTH_MOVE=0x0007` + `MovePathUpdate{key,x,y,path0,dir,angleDeg,pathTail}` + `ParsePacketMoveD4Plain_Asio` (id/size/body≥5; bounds estrito; erro "0xD4:") + `ApplyFrame_PacketMoveD4_Asio(frame, ws, err, missed=nullptr)` (key existente ⇒ dir/angle/target=Data, x/y inalterados; inexistente ⇒ ignora+missed). SYNTAX OK.
- **Golden** (`test_vectors_move_d4_asio/`): generate 7.004 B (single-TU; seed lido dos jsons versionados) · move_d4_asio_vectors.json **688 B sha be448df938000ca30020c09b3f819359468e252210498e44f6dbff53b0108b08** · move_d4_asio_vectors.md **501 B sha 15414dd71df3cccf13a5cdb07905d3a966d14bf22ea3a4fdf6197a999654bced** · compile_and_run.sh 542 B. **Loopback** (`loopback_move_d4_asio/`): embedded 2.336 B 51d16a2f… (sha do json no topo; 5 frames + expected) · loopback_harness.cpp 7.085 B aa0cdd76… (single-TU) · compile_and_run.sh 592 B.
- **Binários REMOVIDOS**: genvec_move_d4_asio 712.968 B sha ac79bf98… · loopback_harness 971.240 B sha 043646e2… (liberados 1.684.208 B; workspace 11.136.038 B c/ .git).
- Nota: localhost only; EVIDENCE intacta; nenhum .dat/binário versionado.

## 76. FASE 1 — 1.3-L: BOTH_POSITION (0x0006 ASIO) OK e equivalente a 0x15 (C1) (golden+loopback; trunc sem efeito; binários removidos)
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (EQUIVALÊNCIA DOS DOIS CANAIS DE POSITION)**: golden aplicou frame12 (C2 79 B) + frame13 (C2 29 B) → **ws=4** → **both_ok (olc 13 B `0600 07000000 C10715 0100 4D 58`)** atualizou **0x0100: x/y/target=(77,88), dir do spawn (1) INTOCADO, missed=0** → **EQUIVALÊNCIA PROVADA**: pkt15_ok (C1 7 B `C10715 0100 4D 58`, golden 1.3-J) aplicado na sequência ⇒ **estado idêntico campo-a-campo** (SameWorld/SameEntity: key/kind/x/y/target/dir/angle/pk/id/classByte/type/buffs; missed15=0) → **both_miss (`…C10715 3333 0909`, key 0x3333 inexistente) → ignored + missed=1, ws segue 4** → **both_trunc (12 B: header diz size=7, body 6 B sem Y) rejeitado** ("0x0006: frame olc invalido (espera 13 B…got 12 B)") com estado intocado (SameWorld snapshot) e missed inalterado. Loopback: server enviou **6 frames [len:u16 LE][frame]** por socket real → client validou tudo (spawn 4 → both_ok → 0x15 equiv → miss 1 → trunc sem efeito).
- **P1/2 (sem rede; classe (1) funcional — campos funcionais provados; header embutido OPACO)**: **BOTH_POSITION = ordinal 6 = 0x0006** (enum class ProtocolHead:uint16_t, ProtocolSend.h :7-:26); ponte **ProtocolSend.cpp :92-:94** `case BOTH_POSITION: ReceiveMovePosition((BYTE*)msg.body.data())` (sha 609e7e935fc88a02…) — MESMO handler do 0x15 (:1746-:1767, 1.3-J). **RESOLUÇÃO DA DIVERGÊNCIA DE MIGRAÇÃO (§75)**: ao contrário do MOVE (struct headerless), PRECEIVE_MOVE_POSITION (WSclient.h :892-:898) mantém **PBMSG_HEADER INCONDICIONAL** ⇒ o body olc (7 B) **CONTÉM os 3 bytes de header embutidos** no início (necessidade estrutural: o cast :1748 lê KeyH no offset 3; sem os 3 bytes ⇒ misparse) — porém o handler **NÃO lê esses 3 bytes** (:1746-:1767 só usa KeyH..Y) ⇒ **OPACOS, valores [NOT RECOVERED]** (golden usa C1 07 15 por convenção clássica; parser NÃO valida — anti-invenção). ⇒ **frame RX = [id:u16=0x0006 LE][size:u32=7 LE][body 7 B [hdr3 opaco][KeyH][KeyL][X][Y]]**, total 13 B. Campos funcionais IDÊNTICOS ao 0x15: **Key BE SEM máscara** :1749; x/y=Data :1762-:1763 E target=Data :1764-:1765; **sem dir/angle**. TX lateral documentado: SendPositionNew :212-:221 envia body **2 B** PMSG_POSITION_SEND{BYTE x; BYTE y} (ProtocolSend.h :44-:48 — self, sem key). Spec `NEW_ERA_PROTOCOL_MVP_BOTH_POSITION_SPEC.md` 3.245 B/39 L sha 3981db8c6cc66bae414b1fa979ac7a39404f1d24b05317e3dc5ec8b0a14347e7.
- **Impl** (`mvp_login_client.cpp` 1.462 L/73.754 B sha 405552252ea0f2b374283551b0daacf220ab52402274fc6b48ed247bbe2f6d05, bloco 1.3-L): `kProto_BOTH_POSITION=0x0006` + `ParseBOTH_POSITION_Asio(frame, MoveUpdate&, err)` (reusa MoveUpdate 1.3-J; valida id/size/body==7; bytes 0..2 opacos; erro "0x0006:") + `ApplyFrame_BOTH_POSITION_Asio(frame, ws, err, missed=nullptr)` (key existente ⇒ x/y/target=Data c/ dir/angle inalterados; inexistente ⇒ ignora+missed; erro ⇒ estado intocado). SYNTAX OK (`-fsyntax-only`).
- **Golden** (`test_vectors_both_position_0006/`): generate_both_position_vectors.cpp 8.875 B/165 L (single-TU; seed frame12/13 lido dos jsons versionados; SameWorld/SameEntity campo-a-campo) · both_position_vectors.json **791 B sha 48017615df55dc341b358c25c05bf04167754e4c6bbf05b8a0ec40c3898e33f6** · both_position_vectors.md **639 B sha a0396b5eb95c007bfe9efd091b375311046e3b03ef876322c5a61e66533f088e** · compile_and_run.sh 580 B. **Loopback** (`loopback_both_position_0006/`): embedded_vectors.h 2.360 B/58 L sha 3a800eb7040e740b59f42ac7a78605d2e0d5dad9d01dc08f97bc28405eaaf7e9 (sha do json no topo; 6 frames + expected) · loopback_harness.cpp 8.204 B/176 L sha ddb9c81fd2200dd52deebe946dec63ad53bcb1abbe1e68b5014e45e428df4d35 (single-TU) · compile_and_run.sh 633 B sha 1749d2e3a4c0032a368d5c808049c0f0440af2c0d5b87f9aea924fcef36fdd32.
- **Binários REMOVIDOS**: genvec_both_position 722.568 B sha aa0ac2f4… · loopback_harness 983.432 B sha bb8fccf9… (liberados 1.706.000 B; workspace 11.323.054 B c/ .git).
- Nota: localhost only; EVIDENCE intacta; nenhum .dat/binário versionado. **1.3-M decidido pelo usuário: attack/skill básico — começar pelo mais simples do dispatcher.**

## 77. FASE 1 — 1.3-M: Damage RX 0x11 OK (lastDamage; miss; trunc sem efeito) + BOTH_MESSAGE 0x000C tunnel OK (inner 0x11); golden+loopback; binários removidos
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (1º COMBAT SIGNAL + TÚNEL)**: golden aplicou frame12 (C2 79 B) + frame13 (C2 29 B) → **ws=4** → **atk_ok (C1 10 B `c10a11 8100 0123 c5 0044`)** ⇒ decode provado: **raw 0x8100 ⇒ Success=b15=1 + Key=0x0100 (&0x7FFF)**, Damage=0x0123, **DT=0xC5 ⇒ type=5 + bDouble + bCombo**, ShieldDamage=0x0044; apply ⇒ **0x0100.lastDamage=0x0123** (c->Hit :3188), missed=0 → **atk_miss (key 0x3333) → ignored + missed=1, ws segue 4** → **atk_trunc (9 B, sem ShieldL) rejeitado** ("0x11: tamanho C1 invalido…") com estado intocado (SameWorld incl. hp/shield/lastDamage) → **TÚNEL: both_ok (olc 16 B `0c00 0a000000` + atk_ok)**: Extract devolve o C1 cru; **probe (lastDamage resetado p/ −1) ⇒ 0x0123 de volta — túnel APLICOU o inner**, missed2=0 → **both_trunc (15 B: size diz 10, body 9) rejeitado** ("0x000C: size u32 LE (10) != body real (9 B)") com estado intocado. Loopback: server enviou **7 frames [len:u16 LE][frame]** por socket real → client validou tudo (spawn 4 → atk_ok → miss 1 → trunc → túnel aplica → túnel trunc sem efeito).
- **P1/2 (sem rede; classe (1) layout provado)**: pacote = **PACKET_ATTACK 0x11 S→C** (define wsclientinline **:26**); dispatcher **case PACKET_ATTACK :13143-:13144** → **ReceiveAttackDamage :2984-:3191**; framing **C1 10 B FIXO** — struct **PRECEIVE_ATTACK :674-:685** (PBMSG_HEADER + 7 BYTEs): [C1][0A][11][KeyH][KeyL][DamH][DamL][DT][SH][SL]. **Key BE c/ máscara** (nuance vs 0x15/0x0006): raw :2995, Success=raw>>15 :2996, Key&=0x7FFF :2997; Damage/ShieldDamage **WORD BE** :3002/:3013; DamageType :3009-:3012 type&0x3F/b6 double/b7 combo (**PBG_ADD_NEWCHAR_MONK_SKILL NÃO definido em nenhum header evidenciado** ⇒ branch #else). Semântica: self Life/Shield clamp-0 :3031-:3041; **c->Hit=Damage :3188** (todos os caminhos ⇒ lastDamage). Debug :3190 label "0x15" = mislabel stale (anotado). **⭐DESCOBERTA: BOTH_MESSAGE = túnel** (ProtocolSend.cpp :99-:137, ordinal 12 do enum :7-:26): body olc = **pacote clássico cru** (autodetect C1/C3|C2/C4 :111-:121, size C2 = MAKEWORD(recv[2],recv[1])) → **TranslateProtocol(head, recv, size, 0) :135** — o canal genérico do build moderno p/ TODO o dispatcher clássico. Irmão não-escolhido: self-dano F3 sub 0x07 (ReceiveDamage :2851-:2865; **bug upstream :2861-:2863 `Shield = ShieldDamage`** — atribui em vez de subtrair; documentado). Spec `NEW_ERA_PROTOCOL_MVP_DAMAGE_RX_SPEC.md` 4.631 B/42 L sha 1a9a3895d6a1d3ad3a175ce2a8c606cc59945b1a6cb0fc1c19430857fdd7abe6.
- **Impl** (`mvp_login_client.cpp` 1.596 L/80.528 B sha 305e493973ba41f483d9c07c519ef0d87f2d4a7691aca380a1534ca02186938a, bloco 1.3-M): `EntityRecord` += `optional<int> hp/shield` (não vêm no viewport; clamp só se populado) + `int lastDamage=-1`; `DamageEvent{key,success,damage,shieldDamage,damageTypeRaw,type,doubleEnable,comboEnable}` + `ParseDamageRxPlain_C1` (C1/0x0A/0x11/10 B; erro "0x11:") + `ApplyFrame_DamageRx_C1(&missed)` + **túnel**: `kProto_BOTH_MESSAGE=0x000C` + `ExtractClassicFromBothMessage_Asio` (id/size/body; erro "0x000C:") + `ApplyFrame_BOTH_MESSAGE_Tunnel_DamageOnly` (escopo deliberado: SÓ inner C1 head 0x11; demais ⇒ erro claro — sem inventar dispatcher). SYNTAX OK.
- **Golden** (`test_vectors_damage_11/`): generate_damage_11_vectors.cpp 12.581 B/236 L (**TWO-TU**: espelho ODR token-idêntico de EntityKind/EntityRecord/WorldState/DamageEvent + prototypes SEM default args; link c/ mvp_login_client.cpp -DNEWERA_MVP_NO_MAIN — padrão 1.3-H retomado por comando; NOTA: exigiu `#include <cstdint>` próprio) · damage_11_vectors.json **895 B sha 703f95cea4513b7a0ba4fa28d52f6ac8b2bd3f7a15aac2613abfcc83e1405bb0** · damage_11_vectors.md **787 B sha 6835082962b53fe851ef9186ae4474183566cd8beca4f5c24b300d7daf4bf03b** · compile_and_run.sh 567 B. **Loopback** (`loopback_damage_11/`): embedded_vectors.h 2.579 B/63 L sha 4d6c31ee9a811b3c58551e4fd023a07e97b18fcb4041a1c2462614396f747148 (sha do json no topo; 7 frames + expected) · loopback_harness.cpp 12.187 B/256 L sha e461d87b66ab65add5cca34f8dd46186f965370bdd9a034d8298d52750bea394 (two-TU) · compile_and_run.sh 623 B sha faac9a7d7d31f6d231b18600d6b2556afe919db36b876ba7673356096dd3bd1b.
- **Binários REMOVIDOS**: genvec_damage_11 849.624 B sha d2e9a817… · loopback_harness 1.076.832 B sha 64263927… (liberados 1.926.456 B; workspace 11.527.668 B c/ .git).
- Nota: localhost only; EVIDENCE intacta; nenhum .dat/binário versionado. **1.3-N decidido pelo usuário: TX mínimo de ataque/skill — o menor request C→S do WSclientinline.**

## 78. FASE 1 — 1.3-N: Attack TX C1 0x11 OK (AT_ATTACK1=0x78) + loopback com resposta damage 0x11; binários removidos
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (1º TX DE COMBATE + CICLO MÍNIMO REQ→RESP)**: golden construiu o request com o BUILDER REAL (`BuildC1_AttackRequestWire(0x0101, dir=3)`) ⇒ **7 B exatos `c1 07 11 0101 78 03`** (memcmp com o esperado da macro) · RESP = attack_ok do golden 1.3-M (**lido de damage_11_vectors.json — fonte primária, zero digitação**) ⇒ apply em ws c/ dummy 0x0100 ⇒ **lastDamage=0x0123, missed=0** · bônus: round-trip `BuildAsio_BOTH_MESSAGE_FromClassicC1 → ExtractClassicFromBothMessage_Asio` (13 B; uso C→S NÃO evidenciado — helper de framing). Loopback (troca RAW, 2 mensagens, sem framing): client enviou 7 B crus → **server validou memcmp golden** (`C1 07 11 01 01 78 03`; divergência imprimiria offset) → server respondeu damage 10 B → client aplicou (`0x0100 lastDamage=0x0123`). **Ciclo mínimo de combate fechado no localhost: TX attack → RX damage → WorldState.**
- **P1/2 (sem rede; classe (1) layout TX provado)**: sender = **macro SendRequestAttack (wsclientinline.h :518-:527)** — o menor attack TX C→S: `spe.Init(0xC1, PACKET_ATTACK=0x11)`; `spe << (Key>>8) << (Key&0xff) << AT_ATTACK1 << Dir`; **AT_ATTACK1=120=0x78** (enum_h :1497; AT_ATTACK2=121; sem ATTACK3 no enum — não inventado) ⇒ **C1 7 B FIXO [C1][07][11][KeyH][KeyL][78][Dir]**; **Key BE SEM máscara no TX** (:524 shift puro); `spe.Send()` defaults ⇒ **C1 plain** (bEncrypt=FALSE ⇒ NÃO C3; StreamPacketEngine :120-:125). Guard "webzen" :520 documentado. Head 0x11 BIDIRECIONAL (RX payload 7 B PRECEIVE_ATTACK ≠ TX payload 4 B macro). **GS RX**: GS_Protocol.cpp :112-:113 `case PROTOCOL_CODE2: gAttack.CGAttackRecv((PMSG_ATTACK_RECV*)lpMsg,aIndex)` (switch(head) :96; Attack.h :4) — PMSG_ATTACK_RECV/PROTOCOL_CODE2 **[NOT RECOVERED]** nos evidenciados (correlação forte 0x11; irmão PROTOCOL_CODE3→CGPositionRecv; case 0x11 :263 = sub-switch MuRummy, não confundir). **Transporte**: global SendPacket [NOT RECOVERED] (path clássico); SendPacketClassic (ProtocolSend.h :146, DataSend cru) ⇒ C1 direto consistente; wrapper C→S C1→0x000C NÃO existe (túnel é RX). Spec `NEW_ERA_PROTOCOL_MVP_ATTACK_TX_SPEC.md` 4.435 B/48 L sha e9942c8f14e89e267bb4b2deae237223b58183fc68dc5211b1b3c93bc1d1f002.
- **Impl** (`mvp_login_client.cpp` 1.666 L/84.276 B sha b3f07b3b968c0662edb62cb2624c32c9ebb13de945b6ddc4462b66168d27a9d2, bloco 1.3-N): `kAt_ATTACK1=0x78` + `BuildC1_AttackRequestWire(targetKey, dir, out, err)` (bytes exatos §macro; SEM máscara — fiel :524; bounds-check; erro "0x11 TX:") + `BuildAsio_BOTH_MESSAGE_FromClassicC1(innerC1, out, err)` (utilitário de framing 0x000C; inner C1 ≥3 B; marcado NÃO wire-real TX). SYNTAX OK.
- **Golden** (`test_vectors_attack_tx_11/`): generate_attack_tx_11_vectors.cpp 8.089 B/158 L (**TWO-TU** espelho ODR + prototypes) · attack_tx_11_vectors.json **411 B sha 4b8c5bfda64299160e899d73ac11f143036286269e6de7e3c4ef15cc35b9c1be** · attack_tx_11_vectors.md **603 B sha 3e14c615b265aa58fc5f938ff2671a5e89de2de801249153090150e864dbf2ca** · compile_and_run.sh 570 B. **Loopback** (`loopback_attack_tx_11/`): embedded_vectors.h 1.064 B/26 L sha 5a5d312eb98d57630bdc76041599a9bc8009f0dcdb722dcf80ebe201524411a1 (sha do json no topo; kReq/kRespDamage + expected) · loopback_harness.cpp 8.027 B/183 L sha 1e9d6ab21f7578a9b1d94943b4a8058043bb34f35f1726c251385ddbd62bee0f (two-TU; troca RAW 7 B→10 B; server memcmp c/ offset-diag) · compile_and_run.sh 620 B sha 0511499c97d705be4535689d4b987bd070a14e01d3835500d4a35f8659f17691.
- **Binários REMOVIDOS**: genvec_attack_tx_11 825.704 B sha 4101e7f3… · loopback_harness 1.050.648 B sha 6fe8e176… (liberados 1.876.352 B; workspace 11.727.699 B c/ .git).
- Nota: localhost only; EVIDENCE intacta; nenhum .dat/binário versionado. **1.3-O decidido pelo usuário: skill TX mínimo (0xDB) OU ack/anim do attack no viewport — o que for mais simples na evidência.**

## 79. FASE 1 — 1.3-O: Skill TX C1 0xDB OK (multi-alvo 9+3·Count; Send(TRUE) documentado) + loopback com resposta damage 0x11; binários removidos
- Data: 2026-09-05 · SEM REDE EXTERNA (somente 127.0.0.1) · **GEN exit 0 + LOOPBACK exit 0 (TX DE SKILL + CICLO REQ→RESP)**: golden construiu o request com o BUILDER REAL (`BuildC1_SkillRequestWire(Type=0x0009, (50,60), serial=1, Count=1, key 0x0101, skillSerial=2)`) ⇒ **12 B exatos `c1 0c db 0009 323c 01 01 0101 02`** (memcmp) · provas extra no gerador: **Count=2 ⇒ 15 B** (loop multi-alvo: +`[KeyH][KeyL][ss]`) e **guard count=0 rejeitado** · RESP = damage 0x11 do golden 1.3-N (**cadeia de fontes primárias**: attack_tx_11_vectors.json#resp_damage_c1_hex) ⇒ apply em ws c/ dummy 0x0100 ⇒ **lastDamage=0x0123, missed=0**. Loopback (troca RAW, 2 mensagens): client enviou 12 B crus → **server memcmp golden** (decode legível: Type/(x,y)/ser/cnt/key/ss) → RESP damage 10 B → client aplicou. **Ciclo skill→damage fechado no localhost.**
- **P1 (sem rede; classe (1) layout TX provado)**: sender = **macro SendRequestMagicAttack (wsclientinline.h :600-:616) == inline :618-:633** (idênticas): `spe.Init(0xC1, PACKET_MAGIC_ATTACK=0xDB :25)`; `spe << HIBYTE(Type) << LOBYTE(Type) << x << y << MakeSkillSerialNumber(&Serial) << Count`; **loop i<Count**: `spe << (Key[i]>>8) << (Key[i]&0xff) << (BYTE)SkillSerial` (3 B/alvo; **SkillSerial POR ALVO, fora de #ifdef — monk-irrelevante**) ⇒ **C1 variável [C1][size=9+3·Count][DB][TypeH][TypeL BE][x][y][Serial][Count] + por alvo [KeyH][KeyL BE sem máscara][SkillSerial]**; Count=1 ⇒ 12 B; size u8 ⇒ Count ≤ 82. **`spe.Send(TRUE)`** :614/:632 — bEncrypt=TRUE (vs attack 1.3-N default FALSE): sender PEDE criptografia; wrap C3 happens na global SendPacket **[NOT RECOVERED]** ⇒ builder produz o **C1 PLAIN pré-encrypt** (bytes de m_byBuffer, autoritativos; anti-invenção). Serial: MakeSkillSerialNumber **[NOT RECOVERED]** (def fora dos evidenciados; contador client-side; valor de TESTE documentado no json). Guard "webzen" :602/:619. **GS RX: case 0x19** :121-:122 → `gSkillManager.CGSkillAttackRecv` (PMSG_SKILL_ATTACK_RECV [NOT RECOVERED]); **0xDB NÃO ocorre no GS_Protocol.cpp** ⇒ tradução 0xDB→0x19 **[NOT RECOVERED]** (camada de conexão — análoga à tradução olc→F3:03 do join 1.3-C). Spec `NEW_ERA_PROTOCOL_MVP_SKILL_TX_SPEC.md` 3.441 B/41 L sha 8d977122aa3bba33df9ae1ab68458b3f5685487b164437ae06b341a499c73a34.
- **Impl** (`mvp_login_client.cpp` 1.726 L/87.557 B sha 83c60157e7f190d0ccfe55e143a914b0baf4b8e3d63222f903e1696ffef4130b, bloco 1.3-O): `BuildC1_SkillRequestWire(skillType, x, y, serial, count, targetKeys*, skillSerial, out, err)` (Type BE, keys BE sem máscara, ss por alvo; guards count 1..82 + bounds; erro "0xDB TX:"). SYNTAX OK.
